...
from settings import docs_path, player_match_table
...
def create_database():
    ...
    matches_data = sql_client.get_data("Matches", "*")
    teams_data = sql_client.get_data("Teams", "*")
    players_matches_data = sql_client.get_data("PlayerMatches", "*")

    # Selecionar informações do lado azul
    blue_side_teams_data = teams_data[teams_data["teamId"] == 100][["matchId", "win"]]
    ...
    combined_data = matches_data.merge(blue_side_teams_data, on="matchId", how="left")
    combined_data = combined_data.merge(side_metrics, on="matchId", how="left")
    combined_data = combined_data.merge(player_metrics_pivot, on="matchId", how="left")
