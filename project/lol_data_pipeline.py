...
def insert_mastery_champions():
    sql = SQLClient()
    player_champion_mastery = sql.get_data("PlayerChampionMastery", "puuid")[
        "puuid"
    ].tolist()
    ...
    puuid_list = df_player_match_table[df_player_match_table["championLevel"].isnull()][
        "puuid"
    ].to_list()
    # Remover duplicatas
    ...
    for puuid in tqdm(puuid_list, desc="Obtendo Maestria dos Campeões"):
        try:
            ...
            sql.insert_dataframe(
                mastery_champions, "PlayerChampionMastery", ["puuid", "championId"]
            )
