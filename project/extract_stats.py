...
def get_rune_stats():
    ...
    champion_ids = [
        champion_info["key"] for champion_info in champions["data"].values()
    ]
    ...
    for rune in runes:
        ...
        secundary_rune_ids.extend(
            [r["id"] for slot in rune["slots"][1:] for r in slot["runes"]]
        )
    ...
    for championId in tqdm(champion_ids, desc="Processando campeões"):
        ...
        for rune_id in primary_rune_ids:
            try:
                win_rate, pick_rate = map(
                    ...
                    stats.get_rune_stats(rune_id).values(),
                )
            except:
        ...
        for secundary_rune_id in secundary_rune_ids:
            try:
                win_rate, pick_rate = map(
                    lambda value: float(value.replace("%", "")),
                    stats.get_secundary_rune_stats(secundary_rune_id).values(),
                )
            except:
                win_rate, pick_rate = -1.0, -1.0
            finally:
    ...
    df_rune_win_rate = df_rune_win_rate.reset_index().rename(
        columns={"index": "championId"}
    )
    df_rune_pick_rate = df_rune_pick_rate.reset_index().rename(
        columns={"index": "championId"}
    )
