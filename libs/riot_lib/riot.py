...
class LeagueOfLegends:
    ...
    def get_league(self, top=300, include_tag=True):
        ...
        challenger_league = self.watcher.league.challenger_by_queue(
            self.region, self.queue
        )
        chall_df = (
            pd.DataFrame(challenger_league["entries"])
        ...
        if top > len(chall_df):
            grandmaster_league = self.watcher.league.grandmaster_by_queue(
                self.region, self.queue
            )
            gm_df = (
                pd.DataFrame(grandmaster_league["entries"])
                .sort_values("leaguePoints", ascending=False)
                .reset_index(drop=True)
            )
        ...
        if top > len(gm_df):
            master_league = self.watcher.league.masters_by_queue(
                self.region, self.queue
            )
            m_df = (
                pd.DataFrame(master_league["entries"])
                .sort_values("leaguePoints", ascending=False)
                .reset_index(drop=True)
            )
            m_df["tier"] = "MASTER"
    ...
    def get_matchlist(self, puuid=None, count=100):
        ...
        try:
            match_ids = self.watcher.match.matchlist_by_puuid(
                region=self.region, puuid=puuid, queue=self.queue, count=count
            )
            return match_ids
    ...
    def get_match(self, match_id):
        ...
        for index in range(2):
            # Champions Picks
            championPickTurn = [
                info["participants"][r + index * 5]["championId"] for r in range(5)
            ]
        ...
        match_data = {
            ...
            "gameEndedInEarlySurrender": info["participants"][0][
        ...
        for index, participant in enumerate(participants):
            ...
            # Runes
            primaryRune = [
                perks["styles"][0]["selections"][i]["perk"] for i in range(4)
            ]
            secudaryRune = [
                perks["styles"][1]["selections"][i]["perk"] for i in range(2)
            ]

            # Extra Information
            rank = self.get_player_rank(summonerId)
            tierRank = (
                rank["tier"][0] + " " + rank["rank"][0]
                if rank is not None
                else "Missing"
            )
            ...
            player_data = {
                ...
                "physicalDamageDealtToChampions": player[
                ...
                "totalDamageShieldedOnTeammates": player[
                ...
                "totalNeutralMinionsKilled": player["totalAllyJungleMinionsKilled"]
                ...
                "damageTakenOnTeamPercentage": challenges[
    ...
    def get_mastery_champion(self, puuid):
        try:
            mastery_champions = self.watcher.champion_mastery.by_puuid(
                self.region, puuid
            )
            ...
            if err.response.status_code == 429:
                ...
                print("SummonerId não encontrado.")
            else:
...
if __name__ == "__main__":
    ...
    match_ids = lol_data.get_matchlist(
        "yibS_oarcTPUlOdhwnSVnah_BiIvGRZP4_fpi_HiJ0Sny1yDnCGzWPyQKariiPj3QVOye8HEOENyZA",
        count=3,
    )
